...
import { CopyTradeModel } from '../models/copytrade.model.js';
import { ALREADY_EXIST } from '../utils/common.js';
...
export async function getCopyTradeText(telegramId: string) {
    let text = '';
    text += `Add or remove wallets from which you'd like to copy trades!`;
...
export async function registerNewCopyTradeAddress(telegramId: string, chain: string, name: string, address: string) {
    ...
    if (null !== fSet) {
        throw new Error(ALREADY_EXIST + `\n<code>${address}</code>\n⚡️ <b>${chain}</b> has already been configured for this address with name <code>${fSet.name}</code>`);
    }

    const nSet = await CopyTradeModel.findOne({ user: user._id, chain: chain, name: name });
    if (null !== nSet) {
        throw new Error(ALREADY_EXIST + `\n<code>${name}</code>\n⚡️ <b>${chain}</b> has already been configured for <code>${nSet.address}</code> with name <code>${name}</code>`);
    }
...
export async function getCopyTradeDetail(copyTradeId: string) {
    ...
    text += `Target Wallet: <code>${fItem.address}</code>\n`;
    text += `\n`;
    text += `🤷‍♀️ <b>Auto Buy</b>\n`;
    ...
    text += `Smart Slippage: ${fItem.autoBuySmartSlippage === '0' ? '❌ Disabled' : fItem.autoBuySmartSlippage === '1' ? `Global (${globalSmartSlippage})` : '✅ Enabled'}\n`;

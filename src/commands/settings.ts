...
import { getBotGeneralConfiguration, getBotPresetMessage, getTokenStatusMessage } from '../utils/messages.js';
import { getSelectedChain, selectChain, selectOtherChain } from '../service/connected.chain.service.js';
import { processError } from '../service/error.js';
import { getBotGasPresetsMarkup, getSettingsMarkup, getTokenPasteMarkup } from '../utils/inline.markups.js';
import { getSettings, updateSettingsInfo } from '../service/settings.service.js';
import { SETTINGS_LISTENER } from '../utils/common.js';
...
const invokeGasPresetSettings = async (ctx: any, chain: string, preset: string) => {
    ...
    try {
        ...
        await refreshSettings(ctx, chain, 'general') //refreshSettingsGasPresets(ctx, chain);
    } catch (err) {
        await processError(ctx, telegramId, err);
    }
}
...
module.exports = (bot: any) => {
	...
	bot.action(RegExp('^' + botEnum.token_refresh.value + '_.+'), async (ctx: any) => {
		const tokenInfoId = ctx.update.callback_query.data.slice(botEnum.token_refresh.value.length + 1)
		try {
			await ctx.answerCbQuery()
		} catch { }
		...
		try {
			...
			if (tokenInfo) {
				...
				await ctx.telegram.editMessageText(ctx.chat.id, ctx.update.callback_query?.message.message_id, undefined, t.text, {
					parse_mode: botEnum.PARSE_MODE_V2,
	...
	bot.action(RegExp('^' + botEnum.settingsPriorityGas.value + '_.+'), async (ctx: any) => {
        const chain = ctx.update.callback_query.data.slice(botEnum.settingsPriorityGas.value.length + 1)
		try {
			await ctx.answerCbQuery()
		} catch  { }
        await ctx.scene.enter(SETTINGS_LISTENER, { input_type: 'priority-gas', msgId: ctx.update.callback_query?.message.message_id, chain: chain });
    })

    bot.action(RegExp('^' + botEnum.settingsPriorityGasRemove.value + '_.+'), async (ctx: any) => {
        const chain = ctx.update.callback_query.data.slice(botEnum.settingsPriorityGasRemove.value.length + 1)
		try {
			await ctx.answerCbQuery()
		} catch  { }
        await ctx.scene.enter(SETTINGS_LISTENER, { input_type: 'priority-gas-remove', msgId: ctx.update.callback_query?.message.message_id, chain: chain });
    })
};

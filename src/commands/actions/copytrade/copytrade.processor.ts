...
import { getCopyTradeDetail, getCopyTradeDetailMarkup, registerNewCopyTradeAddress, updateCopyTradeAddress } from "../../../service/copytrade.service.js"
import { processError } from "../../../service/error.js"
import { ISceneResponse, SceneStageService } from "../../../service/scene.stage.service.js"
import { COPY_TRADE_LISTENER, INVALID_VALUE_SET } from "../../../utils/common.js"
import { getCopyTradeMarkup } from "../../../utils/inline.markups.js"
    ...
    public async processMessage(telegramId: string, sceneContext: ISceneResponse, text: string, ctx: any) {
        ...
        try {
            if (context.inputType === 'copytrade-new-name') {
                ...
                await processCopyTradeNewAddress(telegramId, text, ctx, context)
            }
...
async function processCopyTradeNewAddress(telegramId: string, text: string, ctx: any, context: any) {
    ...
    await registerNewCopyTradeAddress(telegramId, chain, context.name, text);
...
async function processCopyTradeBuyGasPrice(telegramId: string, text: string, ctx: any, context: any) {
    ...
    const copyTradeDB = await CopyTradeModel.findById(context.copyTradeId)
    const minGasPrice = undefined //copyTradeDB.chain === 'ethereum' ? 0 : parseFloat(await chainGasPrice(copyTradeDB.chain))

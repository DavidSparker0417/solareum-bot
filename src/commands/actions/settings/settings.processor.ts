...
export class SettingsListener {
    public async processMessage(telegramId: string, sceneContext: ISceneResponse, text: string, ctx: any) {
        ...
        try {
			...
			else if (context.inputType === 'priority-gas') {
                await processPriorityGas(telegramId, text, ctx, context)
            }
...
async function processPriorityGas(telegramId: string, text: string, ctx: any, context: any) {
    const chain = context.chain
    const priorityGas = parseInt(text)
    if (isNaN(priorityGas) || priorityGas < 0) {
        await new SceneStageService().deleteScene(telegramId)
        throw new Error(INVALID_VALUE_SET + `\nYou must use a valid positive integer. Please try again.`)
    } else {
        await updateSettingsInfo(telegramId, chain, { gasPreset: text })
        ...
        await ctx.telegram.sendMessage(ctx.chat.id, `✅ <b>Priority gas</b> set to <code>${text}</code> <b>SOL</b>!`, {
            parse_mode: botEnum.PARSE_MODE_V2
        })

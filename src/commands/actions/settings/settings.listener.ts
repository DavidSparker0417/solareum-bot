...
import { getSettingsMarkup } from '../../../utils/inline.markups.js';
import { getBotGeneralConfiguration } from '../../../utils/messages.js';
import { updateSettingsInfo } from '../../../service/settings.service.js';
...
settingsListener.enter(async (ctx: any) => {
    ...
    try {
        ...
        if (ctx.scene.state.input_type === 'max-gas-price') {
            const gasPrice = undefined //await chainGasPrice(ctx.scene.state.chain)
            ...
            await ctx.telegram.editMessageText(ctx.chat.id, msgBckId, 0, await getBotGeneralConfiguration(telegramId, chain), {
                parse_mode: botEnum.PARSE_MODE_V2,
                reply_markup: await getSettingsMarkup(telegramId, chain, '')
            })
        ...
        } else if (ctx.scene.state.input_type === 'priority-gas') {
            ret = await ctx.telegram.sendMessage(ctx.chat.id, `Reply to this message with your <b>desired priority gas</b> in <b>SOL</b>.`, {
                parse_mode: botEnum.PARSE_MODE_V2,
                reply_markup: {
                    force_reply: true
                }
        ...
        } else if (ctx.scene.state.input_type === 'priority-gas-remove') {
            await updateSettingsInfo(telegramId, chain, { gasPreset: undefined })

            await ctx.telegram.sendMessage(ctx.chat.id, `❌ Custom priority gas has been deleted!\nSet to <b>avg</b> as <b>default</b>`, {
                parse_mode: botEnum.PARSE_MODE_V2
            });
            ...
            const gasPrice = undefined //await chainGasPrice(chain)

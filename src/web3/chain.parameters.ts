...
import { getBN, getSolAccounts, newSolWeb3 } from './web3.operation.js';
import { queryAndSyncToken } from './multicall.js';
import { getConfiguredChain } from '../service/connected.chain.service.js';
import { DexInfoModel } from '../models/dex.info.model.js';
import { getPriceFromPriceFeed, parsePriceFeedData } from './pricefeed/parse.js';
...
export async function loadChainParameters() {
		...
		for (const chain of chains) {
			try {
				...
				for (const t of chain.tokens) {
					const tokenInfo = await queryAndSyncToken(undefined, chain.name, t, '11111111111111111111111111111111');
	...
	const dbChains: any[] = await ChainModel.find();
	const chains = await Promise.all(dbChains.map(async ch => { return {...ch._doc, web3: await newSolWeb3('', ch.name)}}))

	while (true) {
	    for (const chain of chains) {
	        try {
				const solWeb3 = chain.web3
				while (true) {
					const pFeeds = getSolAccounts(chain.priceFeeds)
					// const accountList = await solWeb3.getProgramAccounts(getSolAccount('FsJ3A3u2vn5cTVofAjvy6y5kwABJAqYWpe4975bi2epH'), 'finalized')
					// console.log('>>>>>>>>>>', accountList)
					const currentSlot = await solWeb3.getSlot('finalized')
					const tarray = await solWeb3.getMultipleAccountsInfo(pFeeds)
					const prices = tarray.map((t, idx) => {
						const priceInfo = getPriceFromPriceFeed(pFeeds[idx], t.data, currentSlot)
						return BN(priceInfo.price || '0').toString()
					})
					if (prices.indexOf('0') < 0) {
						await ChainModel.findOneAndUpdate({ name: chain.name }, { prices: prices })
						break
					}
				}
	        } catch (err) {
...
const enabledFactory = {
	...
	['base']: [
		'0x1b8128c3a1b7d20053d10763ff02466ca7ff99fc', // uniswap v2 factory
	]
}
...
export function getDefaultRouterAndWETH(chain: string, factory: string) {
	const dexFound = defaultDexList[chain]?.find(d => d.factory === factory)
	return {
...
export function getDefaultWETH(chain: string, router: string) {
	const dexFound = defaultDexList[chain]?.find(d => d.router === router)
